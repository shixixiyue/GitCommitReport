        ...
        public static async Task<Rootobject> POST(MaskBase mask, GPTpost content = null)
        {
            if (content == null)
            {
                content = new GPTpost()
                {
                    ...
                    model = Projects.model != null ? Projects.model : Projects.model
                };
            }
            ...
            client.DefaultRequestHeaders.Add("Authorization", $"Bearer {Projects.key}");

            StringContent strcontent = new StringContent(content.ToString(), Encoding.UTF8, "application/json");

            var result = await client.PostAsync(Projects.url, strcontent);
