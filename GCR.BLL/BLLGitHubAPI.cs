...
namespace GCR.BLL
{
    public class BLLGitHubAPI
    {
        ...
        public static async Task<List<GitRepo>> GetRepos(MsGCR_A1 ms)
        {
            ...
            var url = $"{ms.GCR_A1_20}{string.Format(MsGitHubAPI.ReposAPI, ms.GCR_A1_30)}";
            var response = await url
                .WithHeader("Authorization", $"token {ms.GCR_A1_40}")
                .WithHeader("accept", $"application/vnd.github+json")
                .WithHeader("User-Agent", $"GitCommitReport")
        ...
        public static async Task<List<Branche>> GetBranches(MsGCR_A1 ms, List<GitRepo> repos)
        {
            ...
            {
                var url = $"{ms.GCR_A1_20}{string.Format(MsGitHubAPI.BranchesAPI, repo.full_name)}";
                var response = await url
                .WithHeader("Authorization", $"token {ms.GCR_A1_40}")
                .WithHeader("accept", $"application/vnd.github+json")
                .WithHeader("User-Agent", $"GitCommitReport")
                .WithHeader("X-GitHub-Api-Version", $"2022-11-28").GetStringAsync();
        ...
        public static async Task<List<GitCommit>> GetCommit(MsGCR_A1 ms, List<Branche> branches)
        {
            ...
            foreach (var branche in branches)
            {
                ...
                var url = $"{ms.GCR_A1_20}{string.Format(MsGitHubAPI.CommitAPI, branche.gitRepo.full_name, sha)}";
                var response = await url
                .WithHeader("Authorization", $"token {ms.GCR_A1_40}")
                .WithHeader("accept", $"application/vnd.github+json")
                .WithHeader("User-Agent", $"GitCommitReport")
